 ...
 */ 
extern "C" int 
Team2(tModInfo *modInfo) 
{
    printf("Team2 initialized\n");
    memset(modInfo, 0, 10*sizeof(tModInfo));

    modInfo->name    = strdup("Team2");		/* name of the module (short) */
    modInfo->desc    = strdup("Team2");	/* description of the module (can be long) */
...
initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *situation) 
{
    ...
    snprintf(buffer, BUFSIZE, "drivers/Team2/%d/%s", index, trackname);
    *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);

    if (*carParmHandle == NULL) {
        snprintf(buffer, BUFSIZE, "drivers/Team2/%d/default.xml", index);
        *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    }
