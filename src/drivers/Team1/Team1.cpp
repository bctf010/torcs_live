...
static int  pitcmd(int index, tCarElt* car, tSituation *situation);

static MyCar* mycar[BOTS] = { NULL};
 ...
 */ 
extern "C" int 
Team1(tModInfo *modInfo) 
{
    memset(modInfo, 0, 10*sizeof(tModInfo));

    modInfo->name    = strdup("Team1_test");		/* name of the module (short) */
    modInfo->desc    = strdup("Team1's car");	/* description of the module (can be long) */
...
initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *situation) 
{
    ...
    snprintf(buffer, BUFSIZE, "drivers/Team1/%d/%s", index, trackname);
    printf("initTrack 3");
    *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    printf("initTrack 4");
    if (*carParmHandle == NULL) {
        snprintf(buffer, BUFSIZE, "drivers/Team1/%d/default.xml", index);
        *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    }
    printf("initTrack 5");
...
/* Start a new race. */
static void  
newRace(int index, tCarElt* car, tSituation *situation) 
{ 
    printf("Team1 newRace begin\n");
